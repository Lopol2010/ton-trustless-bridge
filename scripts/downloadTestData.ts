...
let NUM_BLOCKS_TO_DOWNLOAD = 5;

let dataToSave = {blocks : [], headerHashes : [], signatures : []}
...
export async function run(provider: NetworkProvider) {
    ...
    let recentKeyBlocks = await getMostRecentKeyBlocks(NUM_BLOCKS_TO_DOWNLOAD);
    saveObjectToJsonFile(dataToSave.blocks, "blocks.json")
    ...
    for (let i = 0; i < recentKeyBlocks.length - 1; i++) {
        ...
        // try {
        await verifyMasterchainBlock(client, newerBlockIdExt, parsedBlock);
        // } catch (err) {
        // console.log("error verification of block: ", blockIdExt.seqno)
        // }
    }
    saveObjectToJsonFile(dataToSave.signatures, "signatures.json")
    saveObjectToJsonFile(dataToSave.headerHashes, "headerHashes.json")
...
async function getMostRecentKeyBlocks(numberOfBlocksToRetrieve: number = 1) {
    ...
    while (numberOfBlocksToRetrieve > 0) {
        ...
        parsedKeyBlocks.push({ parsedBlock, block, blockIdExt: blockIdToStoreInArray });
        numberOfBlocksToRetrieve--;

    }

    dataToSave.blocks = parsedKeyBlocks;
...
async function verifyMasterchainBlock(
    ...
    blockIdToVerify: tonNode_BlockIdExt,
    olderBlockParsed: any
) {

    const validators = olderBlockParsed.extra.custom.config.config.map.get("22");
    ...
    const blockHeader = await liteClient.getBlockHeader(blockIdToVerify);
    ...
    const valSignatures = (await tonweb.provider.send(
        ...
        {
            seqno: blockIdToVerify.seqno,
        }
    )) as any;

    const signatures = valSignatures.signatures as ValidatorSignature[];    
    dataToSave.signatures.push(valSignatures);

    const max_main_validators = olderBlockParsed.extra.custom.config.config.map.get("10").max_main_validators;
    ...
    dataToSave.headerHashes.push({blockHash, fileHash: blockHeader.id.fileHash});
    let totalWeight = 0;
    ...
    console.log("verified block", blockIdToVerify.seqno)
}
...
function saveObjectToJsonFile(obj, filePath) {
    writeFileSync(filePath, JSON.stringify(obj, replacer, 2));
}
